...
param_probabilities_path = 'combined_pruned_layer2.pkl'
parameters_text = ('iter04rnn'+str(param_rnn_size)+
...
output_file = 'output_' + parameters_text + '.txt'
...
def create_model(session, forward_only, use_dropout):
  ...
  model = seq2seq_model.Seq2SeqModel(
      ...
      dtype=dtype)
  ckpt = tf.train.get_checkpoint_state(FLAGS.train_dir.replace('istest1','istest0'))#.replace('recipe_','').replace('restaurant_',''))
...
#-----------------------------------------------------
def test_accuracy(from_data,to_data, sess = None, model=None):
